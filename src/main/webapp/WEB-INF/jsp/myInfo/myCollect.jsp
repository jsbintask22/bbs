    ...
    <div class="panel-body">
        ...
        <div style="margin: 25px; height: 100px; border: 1px solid darkcyan">
            <legend style="height: 45px; line-height: 45px">主题：
                <mark><a href="/topic/detail/<%=collect.getTopic_id()%>" target="_blank"
                         class="mark_a"><%=collect.getTopicVO().getTopic()%>
                </a>
                </mark>&nbsp;&nbsp;<a class="btn btn-danger"
                                      href="${pageContext.request.contextPath}/user/cancelMyCollect?user_id=<%=user.getUser_id()%>&topic_id=<%=collect.getTopic_id()%>">取消收藏</a>
                <strong style="float: right; margin-right: 35px">收藏时间： <%=sdf.format(collect.getCreate_time())%>
                </strong>
            </legend>
            <div class="text-info" style="margin-left: 25px">
                <a href="/otherSpace/<%=collect.getTopicVO().getUser().getUser_id()%>" target="_blank"><h4
                        style="display: inline; float: right; margin-right: 25px">
                    -----&nbsp;&nbsp;<%=collect.getTopicVO().getUser().getUsername()%>
                </h4></a></div>
        </div>
